...
import { sendMailAction } from "@/_action/survey/send-mail";
import { useDraftAutosave } from "@/hooks/useDraftAutosave";
import { AlertCircleIcon, LoaderCircle, Trash2 } from "lucide-react";
import { IBasis } from "@/lib/mail-action/survey/mail";
...
export default function WelcomeForm() {
  ...
  const { restoreDraft, discardDraft } = useDraftAutosave("survey-draft", form, setForm);
  useEffect(() => {
    restoreDraft();
  }, []);
  ...
  return (
    <form className="space-y-4 px-4 md:px-6 max-w-3xl mx-auto py-10">
      <div className="space-y-2">
        <h1 className="text-3xl font-bold">Survey - Send emails</h1>
          ...
          <Label className="flex gap-1.5 items-center" htmlFor="link">
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className=" bg-white border  w-[60%] text-center mx-auto text-[13px] p-[10px] rounded-lg border-[#b5b5b5] ">
                  <p>
                    Enter the link that directs users to where the survey form
                    is.
                  </p>
                </TooltipContent>
              </Tooltip>
